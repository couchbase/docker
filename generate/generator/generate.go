...
func generateDockerfile(variant DockerfileVariant) error {
	...
	if variant.Product == ProductServer {
		...
		params = map[string]any{
			...
			"CB_RELEASE_URL":    variant.mongoCompatibilityReleaseURL(),
			"DOCKER_BASE_IMAGE": variant.dockerBaseImage(),
			"CB_MULTIARCH":      len(variant.Arches) > 1,
		}
	}
...
// rosetta-mdb packages are published under a "rosetta-mdb" release path,
// distinct from the couchbase-mongodb-compatibility product/image name.
func (variant DockerfileVariant) mongoCompatibilityReleaseURL() string {
	if variant.IsStaging {
		return "http://packages-staging.couchbase.com/releases/rosetta-mdb/" + variant.Version
	}
	return "https://packages.couchbase.com/releases/rosetta-mdb/" + variant.Version
}
